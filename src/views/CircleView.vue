...
export default {
  // les données que l'on exporte du formualaire
  data() {
    return {
      circle: {
        rayon: 0,
  ...
  computed: {
    ...mapWritableState(CircleStore, ["list"]), // on utilise notre store et on utulise et le tableau et mapWritableState pour le séter
  },
  methods: {
    CalculCircle(circle) {
      ...
      if (this.circle.rayon > 0) {
        // on pose une condition pour sécuriser l'application, on lance les calculs que si l'entier est supérieur à 0
        circle.perimeter = Math.round(this.circle.rayon * 2 * Math.PI); // on ajoute des clés à notre store qui prennent des valeurs obtenus grâce aux calculs à partir du rayon donnée apr l'utilisateur
        circle.area = Math.round(Math.pow(this.circle.rayon, 2) * Math.PI); //on utilise les proprétés Maths qui natives à Javascripts pour arrondir (Math.round) le resultat de Math.pow qui s'occupe des puissances et Math.PI pour avoir PI
        circle.volume = Math.round(
          (4 / 3) * Math.pow(this.circle.rayon, 3) * Math.PI
        );
